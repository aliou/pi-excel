...
const WriteOperation = Type.Object({
  row: Type.Number({ description: "Data row number (1-indexed, excluding header)" }),
  column: Type.String({ description: "Column name (must match a header)" }),
  value: Type.Any({ description: "New cell value (string, number, boolean, or null)" }),
});
...
const parameters = Type.Object({
  ...
  operations: Type.Array(WriteOperation, {
    description: "List of cell updates. Each specifies a row, column, and new value.",
  }),
});
...
export async function executeWrite(params: {
  ...
  const result = await writeToSheet(params.path, params.sheet, params.operations);
  ...
  return {
    ...
    details: { updatedCells: result.updatedCells, operations: params.operations },
  };
}
...
export function registerWriteTool(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    renderResult(result: any, { expanded, isPartial }: any, theme: any) {
      ...
      const header = theme.fg("success", `Updated ${details.updatedCells} cell(s)`);
      ...
      for (const op of details.operations) {
        const val = op.value === null || op.value === undefined ? "" : String(op.value).replace(/\|/g, "\\|");
