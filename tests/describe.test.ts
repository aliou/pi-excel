...
describe("excel_describe", () => {
  it("describes a simple xlsx workbook", async () => {
    const result = await executeDescribe({ path: path.join(fixtures, "simple.xlsx") });
    ...
    const colNames = wb.sheets[0].columns.map((c: any) => c.name);
    expect(colNames).toEqual(["Name", "Age", "City", "Active"]);

    const colTypes = wb.sheets[0].columns.map((c: any) => c.type);
    expect(colTypes).toEqual(["string", "number", "string", "boolean"]);
  });

  it("describes a simple xls workbook", async () => {
    const result = await executeDescribe({ path: path.join(fixtures, "simple.xls") });
    const wb = result.details.workbook;
  ...
  it("describes a multi-sheet workbook", async () => {
    const result = await executeDescribe({ path: path.join(fixtures, "multi-sheet.xlsx") });
    const wb = result.details.workbook;
  ...
  it("describes a headers-only workbook", async () => {
    const result = await executeDescribe({ path: path.join(fixtures, "headers-only.xlsx") });
    const wb = result.details.workbook;
  ...
  it("throws for non-existent file", async () => {
    await expect(executeDescribe({ path: "/nonexistent/file.xlsx" })).rejects.toThrow(
      "File not found",
    );
  });
